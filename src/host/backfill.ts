 ...
 * `/api/dsh-goal-plus-plus/backfill` when the dashboard first opens
...
export const BACKFILL_ROUTE = '/api/dsh-goal-plus-plus/backfill'
...
export function watchActivityBackfill(ctx: Context): () => void {
  ...
  const routeFiber = ctx.inject(['connection'], (c) => {
    ...
    try {
      ...
      }), 'dsh-goal-plus-plus: backfill route')
    } catch {
  ...
  const runnerFiber = ctx.inject(['sessionQuery', 'sessionProjectionCache', 'sessionPersistence', 'sessions'], (raw) => {
    ...
    const run = async (): Promise<void> => {
      ...
      for (const record of listed) {
        ...
        try {
          ...
          if (isUnsupportedFormat(error)) {
            ...
            refused++
            ctx.logger.debug(`dsh-goal-plus-plus: projection backfill skipped "${header.id}" (${messageOf(error)})`)
          } else {
            ctx.logger.warn(`dsh-goal-plus-plus: projection backfill skipped "${header.id}" (${messageOf(error)})`)
          }
        }
      ...
      if (folded > 0) ctx.logger.info(`dsh-goal-plus-plus: projection rows backfilled for ${folded} session(s)`)
      if (refused > 0) {
        ctx.logger.info(`dsh-goal-plus-plus: projection backfill skipped ${refused} session(s) (legacy log format is not migratable; sources left unchanged)`)
      }
    }
    launch = () => {
      void run().catch((error: unknown) => {
        if (!abort.signal.aborted) ctx.logger.warn(`dsh-goal-plus-plus: projection backfill stopped early (${messageOf(error)})`)
      })
    }
    tryLaunch()
    return () => { abort.abort() }
