...
export const DETAIL_ROUTE = '/api/dsh-goal-plus-plus/detail'
  ...
  ctx.inject(['connection', 'sessions'], (c) => {
    ...
    const handler = async (request: Request): Promise<Response> => {
      ...
      try {
        ...
        return failure('dsh-goal-plus-plus/bad-request', 'body is not JSON')
      }
      if (typeof sessionId !== 'string' || sessionId === '') {
        return failure('dsh-goal-plus-plus/bad-request', 'missing sessionId')
      }
      try {
    ...
    try {
      c.effect(() => register({
        ...
        requestBody: 'buffered',
        fetch: handler,
      }), 'dsh-goal-plus-plus: detail route')
    } catch {
