...
function ownPackageRoot(): string {
  try {
    return dirname(fileURLToPath(import.meta.resolve('dsh-goal-plus-plus/package.json')))
  } catch {
 ...
 * escapes it; a sibling such as `dsh-goal-plus-plus-extra` is reached through `..` too,
...
export function detectHarnessVersion(
  ...
  const running = probeRunningTree((specifier) => {
    ...
    if (!url.startsWith('file:')) throw new Error(`dsh-goal-plus-plus: non-file module URL for ${specifier}`)
  ...
  try {
    ...
    if (typeof homePath === 'function') {
      const req = createRequire((homePath as (...segments: string[]) => string)('profiles', 'dsh-goal-plus-plus-version-probe.cjs'))
