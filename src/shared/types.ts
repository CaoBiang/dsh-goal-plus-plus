...
/**
 * The per-user display-preference vocabulary of the `dsh-goal-plus-plus` settings
 ...
 * live fold state at the `/dsh-goal-plus-plus` `detail` endpoint (host/detail.ts);
 ...
 * tools — e.g. local-link plugins that apply before dsh-goal-plus-plus). The client
 ...
 * `/api/dsh-goal-plus-plus/balance` fetch route (host/balance.ts). `null` on the
