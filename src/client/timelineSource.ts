 ...
 *   `/api/dsh-goal-plus-plus/detail` fetch route (host/detail.ts): one targeted read
...
const DETAIL_ROUTE = '/api/dsh-goal-plus-plus/detail'
...
export function makeDetailFetcher(
  ...
  return async () => {
    ...
    if (!response.ok) throw new Error(`dsh-goal-plus-plus: detail route HTTP ${response.status}`)
    const r = asRecord(await response.json())
    if (r === null || r.ok !== true) throw new Error('dsh-goal-plus-plus: detail read failed')
    if (r.value === null) return null
    const detail = detailOf(r.value)
    if (detail === null) throw new Error('dsh-goal-plus-plus: detail read malformed')
    return detail
  }
}
