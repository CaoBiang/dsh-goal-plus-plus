...
export function makeGoalControl({ t }: ViewKit) {
  return function GoalControl({ sessionId, goal }: { sessionId?: string; goal: GoalState }) {
    ...
    useEffect(() => {
      ...
      setReading(false)
      busy.current = false
      if (!available) { setActivation(undefined); return }
      const get = face.get
      if (get === undefined) {
      ...
      const refresh = () => {
        ...
        void boundedGoalRequest(() => get(sessionId)).then((reply) => {
          ...
          setError(false)
        }).catch(() => { if (!disposed && read === epoch.current) { setActivation(undefined); setError(true) } })
          .finally(() => { if (!disposed && read === epoch.current) setReading(false) })
      }
    ...
    return <div className="gp-control">
      <button type="button" className="lc-gran-btn gp-action gp-goal-toggle" aria-busy={pending || reading}
        disabled={!available || activation === undefined || pending || reading || (action === 'resume' && capped)}
        onClick={() => { void run(action) }}>{t('goal.' + action)}</button>
      {activation === 'unknown' && goal.phase === 'active' && <button type="button" className="lc-gran-btn gp-action"
      ...
      {error && <span role="alert">{t('goal.controlError')} <button type="button" className="lc-gran-btn gp-action"
