...
function apply(ctx: ClientCtx): void {
  ...
  }, 'dsh-goal-plus-plus: dictionaries')
  ...
  // Per-user display preferences: bind the Host-served `dsh-goal-plus-plus`
  ...
  ctx.inject(['settingsScope'], (raw) => {
    ...
    c.effect(() => settings.attach(binder.bind({ namespace: NS })), 'dsh-goal-plus-plus: settings scope')
