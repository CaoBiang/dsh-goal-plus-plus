 ...
 * `/api/dsh-goal-plus-plus/balance` fetch route (host/balance.ts) on every open, so
...
const BALANCE_ROUTE = '/api/dsh-goal-plus-plus/balance'

/** Where the last live figure is remembered across opens, per browser. */
const STORAGE_KEY = 'dsh-goal-plus-plus:platform-balance'
