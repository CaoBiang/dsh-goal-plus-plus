...
describe.skipIf(staging.artifactsMissing())('bundle smoke — the built lib/client.js', () => {
  ...
  const styleTags = (): HTMLElement[] =>
    [...dom.window.document.head.querySelectorAll<HTMLElement>('style[data-plugin="dsh-goal-plus-plus"]')]
  ...
  test('handoff: the bundle registers as dsh-goal-plus-plus with the factory closure', () => {
    assert.ok(handoff !== null, 'bundle must register through __ModuleLoader__.load')
    assert.equal(handoff.id, 'dsh-goal-plus-plus', 'handoff id is the package name')
    assert.equal(typeof handoff.factory, 'function')
    assert.equal(state.plugin?.name, 'dsh-goal-plus-plus')
  ...
  test('CSS channel: plugin-owned style tags, one per sheet, minified', () => {
    ...
    assert.ok(tagIds.every(id => typeof id === 'string' && id.startsWith('dsh-goal-plus-plus/')), 'official data-plugin-css tag ids')
  ...
  test('registrations: bilingual dictionaries, five slots, the /context trigger source', () => {
    assert.ok(state.dicts.get('dsh-goal-plus-plus')?.zh && state.dicts.get('dsh-goal-plus-plus')?.en, 'bilingual dictionaries registered')
