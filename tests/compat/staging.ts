...
export const DSH_REPO = process.env.DSH_REPO || join(USER_HOME, 'dev', 'deepseek-harness')
...
export function ensureHostDeps(baseline: Baseline): void {
  ...
  writeFileSync(join(dir, 'package.json'), JSON.stringify({
    name: `dsh-goal-plus-plus-compat-${baseline.id}`, private: true, type: 'module',
    dependencies: deps,
  }, null, 2))
  ...
  const run = process.platform === 'win32'
    ? spawnSync(process.env.ComSpec ?? 'cmd.exe', ['/d', '/s', '/c', npm, ...args], { cwd: dir, encoding: 'utf8' })
    : spawnSync(npm, args, { cwd: dir, encoding: 'utf8' })
  if (run.status !== 0) {
    const detail = run.error?.message ?? run.stderr ?? run.stdout ?? 'unknown child-process failure'
    throw new Error(`${npm} install ${JSON.stringify(deps)} failed: ${detail.trim().slice(0, 300)}`)
  }
}
