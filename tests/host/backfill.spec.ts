...
/** Fire the captured trigger route (the client's dashboard-open POST). */
async function trigger(route: RouteBox): Promise<Response> {
...
function captureLogs(ctx: Context): LogLine[] {
  ...
  ctx.logger.exporter({
    levels: { root: 3, 'dsh-goal-plus-plus': 3 },
    export(message) {
