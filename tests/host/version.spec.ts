 ...
 */
const ELSEWHERE = join(tmpdir(), 'dsh-goal-plus-plus-plugin-root')
...
beforeAll(() => {
  scratch = mkdtempSync(join(tmpdir(), 'dsh-goal-plus-plus-version-'))
...
describe('detectHarnessVersion — running anchor', () => {
  ...
  test('never probes the CLI package from the running anchor', () => {
    // The fixture pins the CLI at 0.0.1 but the library resolves from the
    // current dev baseline: only the library answer may come back.
