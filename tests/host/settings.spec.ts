...
async function untilRegistered(ctx: Context): Promise<void> {
  ...
  assert.fail('the dsh-goal-plus-plus settings namespace was never registered')
}
...
describe('installSettings', () => {
  test('the namespace is the plugin short name', () => {
    assert.equal(SETTINGS_NAMESPACE, 'dsh-goal-plus-plus')
  })

  test('registers the dsh-goal-plus-plus namespace with schema defaults', async () => {
    const { ctx } = await boot()
    const descriptors = ctx.settings.describe()
    assert.ok(descriptors.some(d => String(d.ns) === 'dsh-goal-plus-plus'), 'the namespace is registered')
  ...
  test('updates flow through the real scope; invalid values reject', async () => {
    ...
    assert.deepEqual(provider.doc['dsh-goal-plus-plus'], { defaultGranularity: 'turn' }, 'the provider persisted the section')
    ...
    assert.deepEqual(
      provider.doc['dsh-goal-plus-plus'],
  ...
  test('a stale persisted preference degrades to the default (loose)', async () => {
    const { ctx } = await boot({ 'dsh-goal-plus-plus': { defaultPlacement: 'window', defaultTrendMode: 'net', defaultToolSort: 'alpha', defaultFileSort: 'alpha', insightsEntry: 'gone' } })
