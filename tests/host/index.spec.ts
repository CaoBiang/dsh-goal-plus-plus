...
describe('dsh-goal-plus-plus host plugin', () => {
  ...
  test('registers all three units and serves the folded views over real appends', async () => {
    ...
    assert.ok(timeline.current.system > 0, 'the V3 system/message contributes system tokens')
    ...
    assert.ok(!('systemTokens' in headers.headers[0]), 'V3 headers do not duplicate the log-sourced system prompt')
  ...
  test('register-hook attribution flows through to the rendered headers', async () => {
    const { ctx } = await boot()
    // dsh-goal-plus-plus booted before any tools service existed; a late-provided
  ...
  test('the split generation with the detail route live: slim wire head + the endpoint serves the collections', async () => {
    ...
    const route = routes.get('/api/dsh-goal-plus-plus/detail')
    assert.ok(route !== undefined && route.fetch !== undefined, 'the detail route registered')
    const response = await route.fetch(new Request('http://dsh.test/api/dsh-goal-plus-plus/detail', {
      method: 'POST',
