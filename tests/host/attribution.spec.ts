...
/** Absolute path of this spec file (a file inside the dsh-goal-plus-plus package). */
...
describe('createToolAttribution', () => {
  ...
  test('unload then re-apply re-wraps the restored original without stacking', async () => {
    ...
    app.provide('tools', { register() { calls++; return () => {} } })
    let attribution!: ToolAttribution
    const first = app.plugin({ name: 'dsh-goal-plus-plus', apply(sub) { createToolAttribution(sub) } })
    await first
    await first.dispose()
  ...
  test('packageNameFrom walks up to the nearest readable package.json', () => {
    ...
    assert.equal(packageNameFrom(malformed), 'dsh-goal-plus-plus', 'a malformed package.json is skipped, the walk continues upward')
