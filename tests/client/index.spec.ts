...
describe('client entry: constants', () => {
  test('name and inject declare the plugin identity and hard dependencies', () => {
    assert.equal(name, 'dsh-goal-plus-plus')
...
describe('client entry: dictionaries', () => {
  test('apply registers the real zh/en dicts; dispose removes them', () => {
    ...
    applyTo(ctx)
    const dicts = ctx.locale.namespaces.get('dsh-goal-plus-plus')
    assert.ok(dicts)
    ...
    assert.equal(ctx.locale.bind('dsh-goal-plus-plus')('tab'), 'Context')
    ctx.dispose()
    assert.equal(ctx.locale.namespaces.has('dsh-goal-plus-plus'), false)
  })

  test('the zh active locale binds the zh dictionary arm', () => {
    const ctx = new TestClientCtx({ locale: 'zh' })
    applyTo(ctx)
    assert.equal(ctx.locale.bind('dsh-goal-plus-plus')('tab'), '上下文')
    ctx.dispose()
  })
})
...
describe('client entry: conversation.view slot', () => {
  test('registers the Context tab whose label follows the locale and whose component renders', async () => {
    ...
    assert.equal(registration.locale, 'dsh-goal-plus-plus')
    assert.equal(registration.label?.(), 'Goal++')

    const el = component({ sessionId: 's1', useProjection: () => undefined }) as ReactElement
    assert.equal(typeof el.type, 'function')
    assert.equal((el.type as { name: string }).name, 'PluginView')
    const m = await mount(el)
    assert.equal(query(m.container, '[role="status"]').textContent, DICT_EN['goal.unavailable'])
    await m.unmount()
    ctx.dispose()
  })
  ...
  test('the tab label translates to zh under an zh locale', () => {
    const ctx = new TestClientCtx({ locale: 'zh' })
    applyTo(ctx)
    assert.equal(ctx.slots.of('conversation.view')[0].registration.label?.(), 'Goal++')
    ctx.dispose()
  })
})
...
describe('client entry: Context Dashboard seats', () => {
  test('registers the sidebar-foot entry and the frame overlay, both rendering', async () => {
    ...
    assert.equal(actions[0].registration.locale, 'dsh-goal-plus-plus')
    ...
    assert.equal(overlays[0].registration.locale, 'dsh-goal-plus-plus')
...
describe('client entry: settingsScope inject', () => {
  ...
  test('armed later: the pending inject runs — scope attached and card slot registered', () => {
    ...
    assert.deepEqual(specs, [{ namespace: 'dsh-goal-plus-plus' }])
...
describe('client entry: right Sidebar Context tab', () => {
  ...
  test('armed later: the pending inject registers the Context tab type and body', () => {
    ...
    assert.equal(definitions[0].id, 'dsh-goal-plus-plus')
    assert.equal(definitions[0].kind, 'dsh-goal-plus-plus')
    assert.equal(definitions[0].title?.(), 'Goal++')
    const bodies = ctx.slots.of('sidebar.right.pane.tab')
    assert.equal(bodies.length, 1)
    assert.equal(bodies[0].registration.key, 'dsh-goal-plus-plus')
    ctx.dispose()
  })
})
...
describe('client entry: settings card slot', () => {
  ...
  test('registers the keyed card; inject exposes the settings store and a set verb', () => {
    ...
    assert.equal(registration.key, 'dsh-goal-plus-plus')
    assert.equal(registration.locale, 'dsh-goal-plus-plus')
...
describe('client entry: dispose', () => {
  test('a full apply unwinds dictionaries and the command source', () => {
    ...
    const ctx = new TestClientCtx({ services: { inputTriggers } })
    applyTo(ctx)
    assert.ok(ctx.locale.namespaces.has('dsh-goal-plus-plus'))
    assert.equal(sources.length, 1)
    ctx.dispose()
    assert.equal(ctx.locale.namespaces.has('dsh-goal-plus-plus'), false)
    assert.equal(sources.length, 0)
