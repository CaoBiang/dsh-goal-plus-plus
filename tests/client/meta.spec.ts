...
describe('meta', () => {
  test('without build defines the dev/test fallbacks apply', async () => {
    ...
    assert.equal(meta.PLUGIN_NAME, 'dsh-goal-plus-plus')
    assert.equal(meta.PLUGIN_VERSION, '0.0.0-dev')
    assert.equal(meta.PLUGIN_REPO, 'https://github.com/CaoBiang/dsh-goal-plus-plus')
    assert.equal(meta.PLUGIN_REPO_SHORT, 'CaoBiang/dsh-goal-plus-plus')
  })
