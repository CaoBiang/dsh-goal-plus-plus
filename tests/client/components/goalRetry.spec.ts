...
test.each(['save', 'toggle', 'cancel'] as const)('polling keeps controls enabled and accepts %s immediately', async action => {
  vi.useFakeTimers()
  const view: RetryView = { ...initial, enabled: true, status: 'waiting', nextAt: Date.now() + 5000 }
  const fetcher = vi.fn(async (_url: unknown, _init: RequestInit) => response(view))
  vi.stubGlobal('fetch', fetcher)
  const m = await mount(h(makeGoalRetry(makeKit()), props))
  const checkbox = query<HTMLInputElement>(m.container, '[type="checkbox"]')
  const save = query<HTMLButtonElement>(m.container, '[type="submit"]')
  const cancel = query<HTMLButtonElement>(m.container, '.gp-retry-details > .gp-control > button')
  let failRead!: (reason: Error) => void
  fetcher.mockImplementationOnce(() => new Promise((_resolve, reject) => { failRead = reject }))
  await act(async () => { await vi.advanceTimersByTimeAsync(1000) })
  ...
  fetcher.mockImplementationOnce(() => new Promise(resolve => { finishWrite = resolve }))
  await click(action === 'save' ? save : action === 'toggle' ? checkbox : cancel)
  assert.equal(signal.aborted, true)
  assert.equal(fetcher.mock.calls.length, 3)
  assert.equal(JSON.parse(String(fetcher.mock.calls[2][1].body)).action, action === 'cancel' ? 'cancel' : 'configure')
  await act(async () => { failRead(Error('aborted')) })
  assert.equal(m.container.querySelector('[role="alert"]'), null)
  assert.equal(checkbox.disabled, true)
  assert.equal(save.disabled, true)
  await act(async () => {
    query(m.container, 'form').dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))
    await vi.advanceTimersByTimeAsync(1000)
  })
  assert.equal(fetcher.mock.calls.length, 3)
  await act(async () => { finishWrite(response(view)) })
  assert.equal(checkbox.disabled, false)
  assert.equal(save.disabled, false)
  await act(async () => { await vi.advanceTimersByTimeAsync(1000) })
...
test.each([true, false])('superseded poll settling after save cannot overwrite settings or report an error (%s)', async success => {
  vi.useFakeTimers()
  const view = { ...initial, enabled: true }
  const fetcher = vi.fn(async (_url: unknown, _init: RequestInit) => response(view))
  vi.stubGlobal('fetch', fetcher)
  const m = await mount(h(makeGoalRetry(makeKit()), props))
  const interval = query<HTMLInputElement>(m.container, 'input[type="number"]')
  await input(interval, '7')
  let finishRead!: (value: ReturnType<typeof response>) => void
  fetcher.mockImplementationOnce(() => new Promise(resolve => { finishRead = resolve }))
  await act(async () => { await vi.advanceTimersByTimeAsync(1000) })
  fetcher.mockResolvedValue(response({ ...view, intervalSeconds: 7 }))
  await click(query(m.container, '[type="submit"]'))
  await act(async () => { finishRead(response(initial, success)) })
  assert.equal(query<HTMLInputElement>(m.container, '[type="checkbox"]').checked, true)
  assert.equal(interval.value, '7')
  assert.equal(m.container.querySelector('[role="alert"]'), null)
  await m.unmount()
})
