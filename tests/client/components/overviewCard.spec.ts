...
describe('OverviewCard', () => {
  ...
  test('the breadcrumb shows group and project; the full path stays on the tip', async () => {
    const m = await mount(h(Card, {
      row: rowOf({ cwd: '/Users/bw/dev/dsh-goal-plus-plus' }),
      group: 'workspace-one',
      costLabel: '—',
      now: NOW,
      onOpen: () => {},
    ...
    assert.equal(crumb.getAttribute('title'), '/Users/bw/dev/dsh-goal-plus-plus')
    ...
    assert.equal(query(m.container, '.lc-ov-crumb-project').textContent, 'dsh-goal-plus-plus')
    await m.unmount()
  })
