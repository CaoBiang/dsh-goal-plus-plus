...
describe('UpgradeGate', () => {
  test('renders both versions and the upgrade prompt (en and zh)', async () => {
    ...
    try {
      ...
      assert.match(text(en.container), /dsh-goal-plus-plus supports DeepSeek Harness v0\.1\.2-rc\.1 or newer\./)
    ...
    try {
      ...
      assert.match(text(zh.container), /dsh-goal-plus-plus 插件支持 DeepSeek Harness v0\.1\.2-rc\.1 或更新版本。/)
