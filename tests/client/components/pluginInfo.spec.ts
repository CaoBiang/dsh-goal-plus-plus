...
describe('PluginInfo', () => {
  test('dev build renders the static rows and never checks the registry', async () => {
    ...
    assert.equal(query(rows[0], '.lc-pi-value').textContent, 'dsh-goal-plus-plus (v0.0.0-dev)')
    assert.equal(rows[0].getAttribute('href'), 'https://github.com/CaoBiang/dsh-goal-plus-plus')
    assert.equal(rows[0].getAttribute('target'), '_blank')
    assert.equal(query(rows[1], '.lc-pi-label').textContent, 'GitHub')
    assert.equal(query(rows[1], '.lc-pi-value').textContent, 'CaoBiang/dsh-goal-plus-plus')
    assert.equal(rows[1].getAttribute('href'), 'https://github.com/CaoBiang/dsh-goal-plus-plus')
    ...
    assert.equal(hint.getAttribute('href'), 'https://github.com/CaoBiang/dsh-goal-plus-plus')
  ...
  test('release build with a newer registry version appends the update chip', async () => {
    ...
    assert.equal(fetchMock.mock.calls[0][0], 'https://registry.npmjs.org/dsh-goal-plus-plus/latest')
    assert.ok(text(m.container).includes('dsh-goal-plus-plus (v1.0.0)'))
