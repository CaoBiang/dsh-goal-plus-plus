...
describe('AgentGraph — cold-relative composition fetch', () => {
  ...
  test('pressure-only relatives fetch their head and re-render composed', async () => {
    ...
    assert.deepEqual(rpc.calls, ['/api/dsh-goal-plus-plus/detail:done'])
    ...
    await flush()
    assert.deepEqual(rpc.calls, ['/api/dsh-goal-plus-plus/detail:done'], 'the cache never re-fetches')
  ...
  test('a failing fetch degrades to the pressure-only ring and never retries', async () => {
    ...
    await flush()
    assert.deepEqual(rpc.calls, ['/api/dsh-goal-plus-plus/detail:done'], 'the sticky failure never re-fetches')
    await m.unmount()
  })
  ...
  test('a head with no occupancy anchor still composes from the fold alone', async () => {
    ...
    assert.deepEqual(rpc.calls, ['/api/dsh-goal-plus-plus/detail:cold'])
