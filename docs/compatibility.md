...
The compact dashboard inside the current-goal card shows observed execution time accumulated only inside explicitly admitted goal turns, durable phase and rounds started, estimated goal-owned added/removed lines, and a numeric count of the current session's running direct subagents. These are independent indicators, not a goal completion percentage. Time advances with observed log events and stops between turns, excluding idle and pause gaps. Older payloads without execution accounting show a dash; code totals retain the file card's retention and estimation limits. The dashboard shares the existing timeline detail store and session-list feed, with no additional polling or local wall-clock extrapolation. Projection state version 23 refolds logs to reconstruct execution accounting. Four cells share one row in wide panes and wrap in narrow panes.

The Goal page's **Subagents** card reuses the optional session-list subscription and direct-child catalog refresh. It lists the current session's direct children (including children created before the current goal), filtered by the harness's live `running` flag through a Running / Ended toggle, with Running selected by default and live counts on both options. Ended means not currently running, not necessarily that the delegated task completed successfully. Each row shows its name, session ID, descriptor type, recorded active time and context tokens when available, and opens the existing session through native navigation. Missing data displays a dash; unavailable lists and failed catalog refreshes remain visible, with manual retry for a failed refresh. Malformed rows are skipped independently.
